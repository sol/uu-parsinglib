...
-- * Classes
   IsLocationUpdatedBy,
-- * Instances for Provides
-- [pSym note]
--
-- pSym is heavily overloaded. (Well, actually it'll work for anything
-- that has a 'Provides' instance, and we have five 'Provides' instances
-- below):
--
--  * (a -> Bool, String, a)
--  * (a,a)
...
-- | A`GenParser` is a parser that is prepared to accept "Data.Listlike" input; hence we can deal with @String@'s, @ByteString@'s, etc.
...
-- | We reserve the identifier `Parser`  for parsers which take a @String@ as input; we  maintain line, column and absolute position info.
type Parser r = P (Str Char String (Int, Int, Int)) r 
...
instance IsLocationUpdatedBy (Int,Int, Int) Char where
   advance (line,pos, abs) c = case c of
                               '\n' ->  (line+1, 0, abs + 1) 
                               '\t' ->  (line  , pos + 8 - (pos-1) `mod` 8, abs + 1)
                               _    ->  (line  , pos + 1, abs + 1)
...
pMunch  p   = pSymExt (Zero Infinite) Nothing  (Munch p "") -- the empty case is handled above
pMunchL p l = pSymExt (Zero Infinite) Nothing  (Munch p l)  -- the empty case is handled above
